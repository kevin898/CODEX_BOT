...
private:
    ...
    CPanelList       *MainTabList, *RiskTabList, *MarginTabList, *SwapsTabList, *TradingTabList, *PersistentList; // PersisntentList can be potentially used for Strategy Tester event tracking in the future.
...
public:
   ...
   ~CPositionSizeCalculator(void);
    ...
    virtual void     OnClickButtonClose();
            void     OnClickEdtPosSize();
            void     OnClickEdtEntryLevel();
            void     OnClickEdtSL();
            void     OnClickEdtTP();
            void     OnClickEdtAccount();
            void     OnClickEdtRiskPIn();
    ...
    CButton          m_OutsideTradeButton;
            ...
            double   DPIScale() {return m_DPIScale;}
...
ON_EVENT(ON_CHANGE, m_ChkTPLockedOnSL, OnChangeChkTPLockedOnSL)
...
bool CPositionSizeCalculator::CreateObjects()
{
    int row_start, h_spacing,
        ...
        panel_end, include_symbols_button_width;
    ...
    include_symbols_button_width = (int)MathRound(95 * m_DPIScale);
...
// Reset
    ...
    long m_RgpIncludeOrders_Width[3] = {tab_button_width, narrow_edit_width, narrow_label_width + v_spacing * 2};
    ...
    long m_RgpIncludeSymbols_Width[3] = {tab_button_width, narrow_edit_width, narrow_label_width + v_spacing * 2};
    ...
    long m_RgpIncludeDirections_Width[3] = {tab_button_width, narrow_edit_width, narrow_label_width + v_spacing * 2};
...
// Reset
    ...
    if (ShowAdditionalMarginSettings)
    {
        ...
        long m_RgpMarginUtilizationBase_Width[3] = {narrow_edit_width, normal_label_width, normal_label_width - h_spacing * 2};
...
// Trading
    ...
    if (ShowMaxParametersOnTrading)
    {
        ...
        if (ShowAdditionalMarginSettings)
        {
            ...
            if (!LabelCreate(TradingTabList, m_LblMaxMarginPercTotal, first_column_start + narrow_label_width + element_height - v_spacing * 2, y, multi_tp_column_start + tab_button_width, y + element_height, "m_LblMaxMarginPercTotal", TRANSLATION_LABEL_TOTAL + ":"))                                         return false;
    ...
    if ((AdditionalTradeButtons == ADDITIONAL_TRADE_BUTTONS_LINE) || (AdditionalTradeButtons == ADDITIONAL_TRADE_BUTTONS_BOTH))
    {
        ...
        if (ShowAdditionalEntryLabel) width = int(width * 1.5);
        ...
        m_OutsideTradeButton.ColorBackground(TradeButtonColorAdjusted);
...
void CPositionSizeCalculator::InitControlsValues()
{
    ...
    m_ChkTPLockedOnSL.Checked(sets.TPLockedOnSL);
    ...
    if (ShowAdditionalMarginSettings)
    {
        ...
        {
            m_EdtMUBStartingBalance.Text(FormatDouble(DoubleToString(sets.MUBStartingBalance, AccountCurrencyDigits), AccountCurrencyDigits));
    ...
    /* Custom Leverage             */ m_EdtCustomLeverage.Text(DoubleToString(sets.CustomLeverage, CustomLeverageDecimals));
...
bool CPositionSizeCalculator::DisplayValues()
{
    ...
    {
        if (!Caption(PanelCaption + " " + TRANSLATION_LABEL_SPREAD + ": " + IntegerToString(SymbolInfoInteger(SymbolForTrading, SYMBOL_SPREAD)))) return false;
    }
    else if (ShowSpread == Ratio) // Spread / SL ratio in percentage.
    {
        ...
        if (SL != 0) if (!Caption(PanelCaption + " " + TRANSLATION_LABEL_SPREAD + ": " + DoubleToString((SymbolInfoInteger(SymbolForTrading, SYMBOL_SPREAD) * _Point / SL) * 100, 2) + "%")) return false;
    }
    else if (!Caption(PanelCaption)) return false;

    //=== Levels
    /* Entry Level    */ if (!m_EdtEntryLevel.PropFlags()) m_EdtEntryLevel.Text(DoubleToString(sets.EntryLevel, _Digits));
    ...
    for (int i = 1; i < sets.TakeProfitsNumber; i++)
    {
        ...
        else
        {
            ...
            if (!AdditionalTPEdits[i - 1].PropFlags()) AdditionalTPEdits[i - 1].Text(tp_text);
        }
    }
    ...
    /* Account Value  */ if (!HideAccSize) if (!m_EdtAccount.PropFlags()) m_EdtAccount.Text(FormatDouble(DoubleToString(AccSize, AccountCurrencyDigits), AccountCurrencyDigits));
    ...
    if (sets.SelectedTab == MainTab)
    {
        ...
        if (sets.CustomBalance <= 0)
        {
            if (AdditionalFunds != 0)
            {
                if (!m_minimized) m_LblAdditionalFundsAsterisk.Show();
                string tooltip = "";
                if (AdditionalFunds > 0) tooltip = "+" + DoubleToString(AdditionalFunds, AccountCurrencyDigits) + " " + TRANSLATION_TOOLTIP_ACCOUNT_SIZE_ASTERISK_ADD;
                else if (AdditionalFunds < 0) tooltip = DoubleToString(-AdditionalFunds, AccountCurrencyDigits) + " " + TRANSLATION_TOOLTIP_ACCOUNT_SIZE_ASTERISK_SUB;
    ...
    /* Risk In        */ if (!m_EdtRiskPIn.PropFlags()) m_EdtRiskPIn.Text(FormatDouble(DoubleToString(DisplayRisk, 2)));
    /* Risk currency  */ if (StringLen(AccountCurrency) > 0) if (!m_LblRiskM.PropFlags()) m_LblRiskM.Text(TRANSLATION_LABEL_RISK + ", " + AccountCurrency + ":");
    ...
    /* Risk Money Out */ if (!m_EdtRiskMRes.Text(FormatDouble(DoubleToString(OutputRiskMoney, AccountCurrencyDigits), AccountCurrencyDigits)))                                return false;
    ...
    /* Position size  */ if (!m_EdtPosSize.PropFlags()) m_EdtPosSize.Text(FormatDouble(DoubleToString(OutputPositionSize, LotStep_digits), LotStep_digits));
    ...
    /* Currency label    */ if (AccountCurrency != "")
        {
            if (!m_LblMarginUtilizedBaseCurrency.Text(TRANSLATION_LABEL_MU_BASE_CURRENCY + ", " + AccountCurrency + ":"))                             return false;
        }
    /* Margin Utilization Base Update */ if (sets.MarginUtilizationBase == MUB_BALANCE) // Only needs updating if not set by user.
        {
            if (!m_EdtMUBStartingBalance.Text(FormatDouble(DoubleToString(AccountInfoDouble(ACCOUNT_BALANCE), AccountCurrencyDigits), AccountCurrencyDigits)))               return false;
        }
        else if (sets.MarginUtilizationBase == MUB_FREE_MARGIN)
        {
            if (!m_EdtMUBStartingBalance.Text(FormatDouble(DoubleToString(AccountInfoDouble(ACCOUNT_MARGIN_FREE), AccountCurrencyDigits), AccountCurrencyDigits)))               return false;
        }
    }
    ...
    double swap_long = SymbolInfoDouble(SymbolForTrading, SYMBOL_SWAP_LONG);
    int swap_long_decimal_places = (int)MathMin(4, CountDecimalPlaces(swap_long));
    double swap_short = SymbolInfoDouble(SymbolForTrading, SYMBOL_SWAP_SHORT);
    int swap_short_decimal_places = (int)MathMin(4, CountDecimalPlaces(swap_short));
    ...
    if (sets.TakeProfitsNumber > 1)
    {
        ...
        if (!TradingTPEdits[0].PropFlags()) TradingTPEdits[0].Text(DoubleToString(sets.TP[0], _Digits));
...
void CPositionSizeCalculator::CalculateSettingsBasedOnLines()
{
        ...
        if (atr != 0) // if atr == 0, the indicator data hasn't loaded yet.
        {
            if (sets.ATRMultiplierSL > 0)
            {
                ...
                if (sets.TradeDirection == Long)
                {
                    ...
                    if (sets.SpreadAdjustmentSL) sets.StopLossLevel -= (SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) - SymbolInfoDouble(SymbolForTrading, SYMBOL_BID)); // Adjust for spread to preserve probabilities.
                }
                else
                {
                    sets.StopLossLevel = sets.EntryLevel + sl;
            ...
            if (sets.ATRMultiplierTP > 0)
            {
                ...
                if (sets.StopLossLevel < sets.EntryLevel)
                {
                    ...
                    if (sets.SpreadAdjustmentTP) sets.TakeProfitLevel -= (SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) - SymbolInfoDouble(SymbolForTrading, SYMBOL_BID)); // Adjust for spread to preserve probabilities.
                }
                else
                {
                    sets.TakeProfitLevel = sets.EntryLevel - tp;
    ...
    if (sets.EntryType == Instant)
    {
        ...
        if ((SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) > 0) && (SymbolInfoDouble(SymbolForTrading, SYMBOL_BID) > 0))
        {
            // Long entry
            if (tStopLossLevel < SymbolInfoDouble(SymbolForTrading, SYMBOL_BID)) tEntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
            // Short entry
            else if (tStopLossLevel > SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK)) tEntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_BID);
            // Undefined entry
            else
            {
                // Move tEntryLevel to the nearest line.
                if ((tEntryLevel - SymbolInfoDouble(SymbolForTrading, SYMBOL_BID)) < (tEntryLevel - SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK))) tEntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_BID);
                else tEntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
            }
...
bool CPositionSizeCalculator::CreateOutsideButton(CButton &Btn, int X1, int Y1, int X2, int Y2, string Name, string Text, string Tooltip = "\n")
{
    ...
    // Used in the Strategy Tester visual mode to find this button's ID.
    CStringForList *obj = new CStringForList;
    obj.Name = Name;
    obj.Obj = GetPointer(Btn);
...
void CPositionSizeCalculator::RefreshValues()
{
    ...
    LastRecalculationTime = GetTickCount64();
}
...
void CPositionSizeCalculator::OnClickBtnOrderType()
{
    ...
    else if (sets.EntryType == Pending)
    {
        ...
        if (sets.ShowLines)
        {
            ...
            if (ShowMainLineLabels) ObjectSetInteger(ChartID(), ObjectPrefix + "EntryLabel", OBJPROP_TIMEFRAMES, OBJ_ALL_PERIODS);
        }
    }
    else if (sets.EntryType == StopLimit)
    {
...
void CPositionSizeCalculator::OnClickBtnLines()
{
    ...
    if (sets.ShowLines)
    {
        ...
        if ((ShowMainLineLabels) && (sets.EntryType != Instant))
        {
        ...
        if ((ShowMainLineLabels) && (sets.EntryType == StopLimit))
        {
...
void SwitchEntryDirection()
{
    ...
    else
    {
        ...
        if (sets.EntryType == Instant) sets.EntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
...
void CPositionSizeCalculator::OnClickBtnCommissionType()
{
    ...
    m_EdtCommissionSize.Text(DoubleToString(sets.CommissionPerLot, CommissionDecimals));
...
void CPositionSizeCalculator::OnClickBtnTakeProfitsNumberAdd()
{
    ...
    if (sets.TakeProfitsNumber == 2)
    {
        ...
        {
            ButtonCreate(MainTabList, m_BtnTakeProfitsNumberRemove, first_column_start, y, first_column_start + v_spacing * 4 - 1, y + element_height, name, "x", TRANSLATION_TOOLTIP_TAKEPROFIT_REMOVE);
            if (DetectedColorMode)
            {
    ...
    if ((ShowMainLineLabels) || (ShowAdditionalTPLabel)) Initialization(); // Creates necessary line labels.
...
void CPositionSizeCalculator::OnEndEditEdtEntryLevel()
{
    ...
    m_EdtEntryLevel.PropFlags(0); // Reset the 'being edited' flag.
...
void CPositionSizeCalculator::OnEndEditEdtTP()
{
    ...
    m_EdtTP.PropFlags(0); // Reset the 'being edited' flag.
    StringReplace(s, ",", "."); // Replace comma with period for normal double conversion. 
    if (!sets.TPDistanceInPoints)
    {
    ...
    if (tTakeProfitLevel != sets.TakeProfitLevel)
    {
        ...
        {
            if (sets.TakeProfitLevel == 0) sets.ATRMultiplierTP = 0;
            else
            {
                double buf[1] = {0};
                if (ATR_handle != INVALID_HANDLE) CopyBuffer(ATR_handle, 0, (int)ATRCandle, 1, buf);
...
void CPositionSizeCalculator::OnChangeChkTPLockedOnSL()
{
    if (sets.TPLockedOnSL != m_ChkTPLockedOnSL.Checked())
    {
        sets.TPLockedOnSL = m_ChkTPLockedOnSL.Checked();
        if (!sets.TPLockedOnSL)
        {
            ObjectSetInteger(ChartID(), ObjectPrefix + "TakeProfitLine", OBJPROP_SELECTABLE, true);
...
void CPositionSizeCalculator::OnEndEditEdtAccount()
{
    ...
    m_EdtAccount.PropFlags(0); // Reset the 'being edited' flag.
...
void CPositionSizeCalculator::OnEndEditEdtRiskPIn()
{
    ...
    m_EdtRiskPIn.PropFlags(0); // Reset the 'being edited' flag.
...
void CPositionSizeCalculator::OnEndEditEdtRiskMIn()
{
    ...
    m_EdtRiskMIn.PropFlags(0); // Reset the 'being edited' flag.
    // Try to swap , for . in the "normal" decimal separator positions only. Other commas will be simply removed.
    if (StringGetCharacter(text, StringLen(text) - 2) == ',') StringSetCharacter(text, StringLen(text) - 2, '.');
    if (StringGetCharacter(text, StringLen(text) - 3) == ',') StringSetCharacter(text, StringLen(text) - 3, '.');
    StringReplace(text, ",", "");
...
{
    string text = m_EdtPosSize.Text();
    m_EdtPosSize.PropFlags(0); // Reset the 'being edited' flag.
    // Try to swap , for . in the "normal" decimal separator positions only. Other commas will be simply removed.
    if (StringGetCharacter(text, StringLen(text) - 2) == ',') StringSetCharacter(text, StringLen(text) - 2, '.');
    if (StringGetCharacter(text, StringLen(text) - 3) == ',') StringSetCharacter(text, StringLen(text) - 3, '.');
    StringReplace(text, ",", "");
...
void CPositionSizeCalculator::OnEndEditEdtCustomLeverage()
{
    ...
    if (CustomLeverage != sets.CustomLeverage)
    {
        CustomLeverage = sets.CustomLeverage;
        CustomLeverageDecimals = CountDecimalPlaces(CustomLeverage);
        m_EdtCustomLeverage.Text(DoubleToString(sets.CustomLeverage, CustomLeverageDecimals));
        RefreshValues();
    }
}
...
void CPositionSizeCalculator::OnClickEdtPosSize() // Clicking on an edit starts its editing process.
{
    m_EdtPosSize.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtEntryLevel() // Clicking on an edit starts its editing process.
{
    if (m_EdtEntryLevel.ReadOnly()) return; // Skip if the Edit is read-only.
    m_EdtEntryLevel.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtSL() // Clicking on an edit starts its editing process.
{
    m_EdtSL.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtTP() // Clicking on an edit starts its editing process.
{
    m_EdtTP.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtAccount() // Clicking on an edit starts its editing process.
{
    if (m_EdtAccount.ReadOnly()) return; // Skip if the Edit is read-only.
    m_EdtAccount.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtRiskPIn() // Clicking on an edit starts its editing process.
{
    m_EdtRiskPIn.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtRiskMIn() // Clicking on an edit starts its editing process.
{
    m_EdtRiskMIn.PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickAdditionalTPEdit(int i) // Clicking on an edit starts its editing process.
{
    AdditionalTPEdits[i - 1].PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}

void CPositionSizeCalculator::OnClickEdtTradingTPEdit1() // Clicking on an edit starts its editing process.
{
    // Only for the main TP:
    TradingTPEdits[0].PropFlags(1); // Edit is being edited, do not modify it anywhere else.
}
...
bool CPositionSizeCalculator::SaveSettingsOnDisk(string symbol = "")
{
    ...
    FileWrite(fh, "IncludeOrders");
    FileWrite(fh, IntegerToString(sets.IncludeOrders));
    ...
    FileWrite(fh, "IncludeSymbols");
    FileWrite(fh, IntegerToString(sets.IncludeSymbols));
    FileWrite(fh, "IncludeDirections");
    FileWrite(fh, IntegerToString(sets.IncludeDirections));
    ...
    FileWrite(fh, "MaxRiskPercentage");
    FileWrite(fh, DoubleToString(sets.MaxRiskPercentage));
    ...
    if (GlobalVariableGet("PS-" + IntegerToString(ChartID()) + "-Parameters") > 0)
    {
        ...
        FileWrite(fh, "Parameter_DefaultIncludeOrders");
        FileWrite(fh, IntegerToString(DefaultIncludeOrders));
        ...
        FileWrite(fh, "Parameter_DefaultIncludeSymbols");
        FileWrite(fh, IntegerToString(DefaultIncludeSymbols));
        FileWrite(fh, "Parameter_DefaultIncludeDirections");
        FileWrite(fh, IntegerToString(DefaultIncludeDirections));
...
bool CPositionSizeCalculator::LoadSettingsFromDisk()
{
    ...
    while (!FileIsEnding(fh))
    {
        ...
        else if (var_name == "IncludeOrders")
            sets.IncludeOrders = (INCLUDE_ORDERS)StringToInteger(var_content);
        ...
        else if (var_name == "IncludeSymbols")
            sets.IncludeSymbols = (INCLUDE_SYMBOLS)StringToInteger(var_content);
        else if (var_name == "IncludeDirections")
            sets.IncludeDirections = (INCLUDE_DIRECTIONS)StringToInteger(var_content);
        ...
        else if (GlobalVariableGet("PS-" + IntegerToString(ChartID()) + "-Parameters") > 0)
        {
            ...
            else if (var_name == "Parameter_DefaultIncludeOrders")
            {
                if ((INCLUDE_ORDERS)StringToInteger(var_content) != DefaultIncludeOrders) sets.IncludeOrders = DefaultIncludeOrders;
            }
            ...
            else if (var_name == "Parameter_DefaultIncludeSymbols")
            {
                if ((INCLUDE_SYMBOLS)StringToInteger(var_content) != DefaultIncludeSymbols) sets.IncludeSymbols = DefaultIncludeSymbols;
            }
            else if (var_name == "Parameter_DefaultIncludeDirections")
            {
                if ((INCLUDE_DIRECTIONS)StringToInteger(var_content) != DefaultIncludeDirections) sets.IncludeDirections = DefaultIncludeDirections;
            }
            ...
            {
                if (StringToDouble(var_content) != DefaultMaxMarginPerc) sets.MaxMarginPerc = DefaultMaxMarginPerc;
            }
            else if (var_name == "Parameter_DefaultMaxMarginPercTotal")
            {
                if (StringToDouble(var_content) != DefaultMaxMarginPercTotal) sets.MaxMarginPercTotal = DefaultMaxMarginPercTotal;
            }
            else if (var_name == "Parameter_DefaultMaxMarginPercPerSymbol")
            {
                if (StringToDouble(var_content) != DefaultMaxMarginPercPerSymbol) sets.MaxMarginPercPerSymbol = DefaultMaxMarginPercPerSymbol;
            }
            ...
            {
                if (StringToDouble(var_content) != DefaultMUBStartingBalance) sets.MUBStartingBalance = DefaultMUBStartingBalance;
            }
        }
    }
...
// A weird fix for a weird bug. The TP line will get deselected by CWndContainer::Hide().
bool tp_line_selected = ObjectGetInteger(ChartID(), ObjectPrefix + "TakeProfitLine", OBJPROP_SELECTED);
    Hide();
ObjectSetInteger(ChartID(), ObjectPrefix + "TakeProfitLine", OBJPROP_SELECTED, tp_line_selected);
    Show();
...
void CPositionSizeCalculator::UpdateTradingTPEdit(int i)
{
    ...
    if (i == 0) TradingTPEdits[0].PropFlags(0); // Reset the 'being edited' flag.
    StringReplace(s, ",", "."); // Replace comma with period for normal double conversion. 
...
void CPositionSizeCalculator::UpdateAdditionalTPEdit(int i)
{
    ...
    AdditionalTPEdits[i - 1].PropFlags(0); // Reset the 'being edited' flag.
    StringReplace(s, ",", "."); // Replace comma with period for normal double conversion. 
    // First, do the formatting.
    double new_value = StringToDouble(s);
...
// Resets the m_ChkTPLockedOnSL checkbox. To be called from outside the class.
void CPositionSizeCalculator::ResetChkTPLockedOnSL()
{
...
void CPositionSizeCalculator::DummyObjectSelect(string dummy_name = "DummyObject")
{
    ...
    ObjectSetInteger(ChartID(), ObjectPrefix + dummy_name, OBJPROP_COLOR, clrNONE);
}
...
void Initialization()
{
    ...
    if ((sets.StopLossLevel >= -_Point) && (sets.StopLossLevel <= _Point))
    {
        if (sets.TradeDirection == Long)
        {
            if (sets.EntryLevel == 0) sets.EntryLevel = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
    ...
    if (!DisableStopLimit)
    {
        ...
        if (ShowMainLineLabels)
        {
            ...
            ObjectSetInteger(ChartID(), ObjectPrefix + "StopPriceLabel", OBJPROP_COLOR, sp_label_font_color);
            ObjectSetInteger(ChartID(), ObjectPrefix + "StopPriceLabel", OBJPROP_FONTSIZE, font_size);
            ObjectSetString(ChartID(), ObjectPrefix + "StopPriceLabel", OBJPROP_FONT, font_face);
...
void RecalculatePositionSize()
{
    ...
    if ((AccountInfoString(ACCOUNT_CURRENCY) == "") || (!TerminalInfoInteger(TERMINAL_CONNECTED)) || (!SymbolInfoInteger(Symbol(), SYMBOL_SELECT)) || (!SymbolInfoInteger(SymbolForTrading, SYMBOL_SELECT))) return;

    GetSymbolAndAccountData();

    Bid = SymbolInfoDouble(SymbolForTrading, SYMBOL_BID);
    Ask = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
    ...
    double StopLevel = SymbolInfoInteger(SymbolForTrading, SYMBOL_TRADE_STOPS_LEVEL) * _Point;
    WarningEntry = "";
    WarningSL = "";
    WarningSP = "";
    double AskBid = 0;
    ...
    {
        DrawLineLabel(ObjectPrefix + "EntryAdditionalLabel", FormatDouble(DoubleToString(OutputPositionSize, LotStep_digits), LotStep_digits), tEntryLevel, true);
        if (sets.ShowLines) ObjectSetInteger(ChartID(), ObjectPrefix + "EntryAdditionalLabel", OBJPROP_TIMEFRAMES, OBJ_ALL_PERIODS);
        else ObjectSetInteger(ChartID(), ObjectPrefix + "EntryAdditionalLabel", OBJPROP_TIMEFRAMES, OBJ_NO_PERIODS);
    }
    if ((ShowMainLineLabels) && (sets.EntryType != Instant))
    ...
    for (int i = 1; i < sets.TakeProfitsNumber; i++)
    {
        if (ShowMainLineLabels)
        {
            DrawLineLabel(ObjectPrefix + "TakeProfitLabel" + IntegerToString(i), IntegerToString((int)MathRound((MathAbs(add_tTakeProfitLevel[i - 1] - tEntryLevel) / _Point))), add_tTakeProfitLevel[i - 1]);
        ...
        if (ShowAdditionalTPLabel)
        {
            ...
            DrawLineLabel(ObjectPrefix + "TPAdditionalLabel" + IntegerToString(i), label_text, add_tTakeProfitLevel[i - 1], true);
    ...
    if ((ShowMainLineLabels) && (sets.EntryType == StopLimit))
    {
        DrawLineLabel(ObjectPrefix + "StopPriceLabel", IntegerToString((int)MathRound((MathAbs(tStopPriceLevel - tEntryLevel) / _Point))), tStopPriceLevel);
...
void CalculateRiskAndPositionSize()
{
    ...
    if ((AccountCurrency == BaseCurrency) && ((CalcMode == SYMBOL_CALC_MODE_FOREX) || (CalcMode == SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE)) && (tStopLossLevel != 0))
    {
        ...
        if (tStopLossLevel < tEntryLevel)
        {
            current_rate = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
        }
        else if (tStopLossLevel > tEntryLevel)
        {
            current_rate = SymbolInfoDouble(SymbolForTrading, SYMBOL_BID);
        }
    ...
    {
        OutputPositionSize = AdjustPositionSizeByMinMaxStep(OutputPositionSize);
    }
    if ((CapMaxPositionSizeBasedOnMargin) && (OutputPositionSize > OutputMaxPositionSize)) OutputPositionSize = OutputMaxPositionSize; // Cap with max position size by free margin if necessary.
    ...
    if (tTakeProfitLevel > 0)
    {
        ...
        {
            InputRR = TRANSLATION_LABEL_WARNING_TOO_CLOSE;
...
void CalculatePortfolioRisk(const CALCULATE_RISK_FOR_TRADING_TAB calculate_risk_for_trading_tab = CALCULATE_RISK_FOR_TRADING_TAB_NO)
{
    ...
    double volume = 0;
    if ((sets.IncludeOrders != INCLUDE_ORDERS_OPEN) || (calculate_risk_for_trading_tab))
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            {
                if ((OrderGetString(ORDER_SYMBOL) != SymbolForTrading) && (sets.IncludeSymbols == INCLUDE_SYMBOLS_CURRENT)) continue;
                else if ((OrderGetString(ORDER_SYMBOL) == SymbolForTrading) && (sets.IncludeSymbols == INCLUDE_SYMBOLS_OTHER)) continue;

                if (((OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_STOP) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_STOP_LIMIT)) && (sets.IncludeDirections == INCLUDE_DIRECTIONS_SELL)) continue;
                if (((OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_STOP) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_STOP_LIMIT)) && (sets.IncludeDirections == INCLUDE_DIRECTIONS_BUY)) continue;

                if ((OrderGetDouble(ORDER_SL) == 0) && (sets.IgnoreOrdersWithoutSL)) continue;
                if ((OrderGetDouble(ORDER_TP) == 0) && (sets.IgnoreOrdersWithoutTP)) continue;
            }
            else if ((OrderGetString(ORDER_SYMBOL) != SymbolForTrading) && (calculate_risk_for_trading_tab == CALCULATE_RISK_FOR_TRADING_TAB_PER_SYMBOL)) continue;
            ...
            if ((OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_LIMIT) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_STOP) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_BUY_STOP_LIMIT))
            {
                ...
                if (OrderGetDouble(ORDER_SL) == 0)
                {
                    ...
                    PointsLoss = OrderGetDouble(ORDER_PRICE_OPEN);
                }
                else
                {
            ...
            // Sell orders.
            else if ((OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_LIMIT) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_STOP) || (OrderGetInteger(ORDER_TYPE) == ORDER_TYPE_SELL_STOP_LIMIT))
            {
                // No stop-loss.
                if (OrderGetDouble(ORDER_SL) == 0)
                {
                ...
                if (OrderGetDouble(ORDER_TP) == 0)
                {
                    ...
                    PointsReward = OrderGetDouble(ORDER_PRICE_OPEN);
                }
                else
                {
    ...
    if ((sets.IncludeOrders != INCLUDE_ORDERS_PENDING) || (calculate_risk_for_trading_tab))
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            if (PositionSelectByTicket(PositionGetTicket(i)))
            {
                ...
                if (!calculate_risk_for_trading_tab)
                {
                    if ((PositionGetString(POSITION_SYMBOL) != SymbolForTrading) && (sets.IncludeSymbols == INCLUDE_SYMBOLS_CURRENT)) continue;
                    else if ((PositionGetString(POSITION_SYMBOL) == SymbolForTrading) && (sets.IncludeSymbols == INCLUDE_SYMBOLS_OTHER)) continue;

                    if ((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) && (sets.IncludeDirections == INCLUDE_DIRECTIONS_SELL)) continue;
                    if ((PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL) && (sets.IncludeDirections == INCLUDE_DIRECTIONS_BUY)) continue;
                    
                    if ((PositionGetDouble(POSITION_SL) == 0) && (sets.IgnoreOrdersWithoutSL)) continue;
                    if ((PositionGetDouble(POSITION_TP) == 0) && (sets.IgnoreOrdersWithoutTP)) continue;
                }
                else if ((PositionGetString(POSITION_SYMBOL) != SymbolForTrading) && (calculate_risk_for_trading_tab == CALCULATE_RISK_FOR_TRADING_TAB_PER_SYMBOL)) continue;
                ...
                if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
                {
                    ...
                    {
                        // Losing all the current value.
                        PointsLoss = PositionGetDouble(POSITION_PRICE_OPEN);
                    }
                    else
                    {
                        // Stop-loss below open price.
                ...
                else if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_SELL)
                {
                    ...
                    {
                        // Potential reward is current value.
                        PointsReward = PositionGetDouble(POSITION_PRICE_OPEN);
                    }
                    else
                    {
...
void CalculateMargin()
{
    ...
    SymbolInfoMarginRate(SymbolForTrading, OrderType, initial_margin_rate, maintenance_margin_rate);
    ...
    if (AccountCurrency != MarginCurrency) CurrencyCorrectionCoefficient = CalculateAdjustment(Loss, MarginCurrency, AccountCurrency, SymbolForTrading);
    ...
    else
    {
        ...
        if (NormalizeDouble(HedgedRatio, 2) < 1.00)
        {
            ...
            // There is a position to hedge and new position is in opposite direction.
            if ((volume > 0) && (type != PositionDirection))
            {
                ...
                // If initial margin requirement is bigger than maintenance margin requirement, need to calculate a separate position margin value
                // to check if there is enough necessary free margin before placing a trade.
                if ((InitialMargin > MaintenanceMargin) || (initial_margin_rate > maintenance_margin_rate))
                {
                    if (InitialMargin > 0) ContractSize = InitialMargin;
    ...
    {
        CalculateMarginUtilizationSymbol(Leverage, initial_margin_rate, maintenance_margin_rate);
    }
    ...
    if (!CalculateUnadjustedPositionSize) // If need to adjust to broker's restrictions.
    {
        OutputMaxPositionSize = AdjustPositionSizeByMinMaxStep(MaxPositionSizeByMargin);
    }
...
void CalculateMarginUtilizationSymbol(double leverage, double initial_margin_rate, double maintenance_margin_rate)
{
    MarginUtilizedCurrentSymbolCurrency = 0;
    if (AccountMarginMode != ACCOUNT_MARGIN_MODE_RETAIL_HEDGING) // Netting
    {
        if (PositionSelect(SymbolForTrading))
        {
            double existing_open_price = 1;
            double existing_volume = PositionGetDouble(POSITION_VOLUME);
            if ((CalcMode == SYMBOL_CALC_MODE_CFD) || (CalcMode == SYMBOL_CALC_MODE_CFDINDEX) ||
            ...
            MarginUtilizedCurrentSymbolCurrency = CalculateMarginByVolume(existing_volume, existing_open_price, leverage, initial_margin_rate, maintenance_margin_rate);
        }
    }
    else // Hedging
    {
        ...
        for (int i = 0; i < total; i++)
        {
            ...
            if (PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY)
            {
                ...
                {
                    open_price = volume * open_price + PositionGetDouble(POSITION_VOLUME) * PositionGetDouble(POSITION_PRICE_OPEN); // Volume-weighted average price.
                    volume += PositionGetDouble(POSITION_VOLUME);
                    open_price /= volume; // Divide by new volume to normalize the price.
                }
                else if (type == POSITION_TYPE_SELL)
                {
                    if (HedgedRatio > 0)
                    {
                        double removed_volume = MathMin(volume, PositionGetDouble(POSITION_VOLUME));
                        margin_sum += CalculateMarginByVolume(removed_volume * HedgedRatio, open_price, leverage, initial_margin_rate, maintenance_margin_rate); // Sum up removed volume's margin.
                    }
                    volume -= PositionGetDouble(POSITION_VOLUME);
                    if (volume < 0)
                    {
        ...
        margin_sum += CalculateMarginByVolume(volume, open_price, leverage, initial_margin_rate, maintenance_margin_rate);
        MarginUtilizedCurrentSymbolCurrency = margin_sum;
    }
}

// Used only by CalculateMarginUtilizationSymbol().
double CalculateMarginByVolume(double existing_volume, double existing_open_price, double leverage, double initial_margin_rate, double maintenance_margin_rate)
{
    double ExistingPositionMargin = (existing_volume * ContractSize * existing_open_price / leverage) * maintenance_margin_rate;
    // Otherwise, no need to adjust margin.
    double CurrencyCorrectionCoefficient_existing = 1;
...
void GetSwapData()
{
    ENUM_SYMBOL_SWAP_MODE swap_mode = (ENUM_SYMBOL_SWAP_MODE)SymbolInfoInteger(SymbolForTrading, SYMBOL_SWAP_MODE);
    double swap_long = SymbolInfoDouble(SymbolForTrading, SYMBOL_SWAP_LONG);
    double swap_short = SymbolInfoDouble(SymbolForTrading, SYMBOL_SWAP_SHORT);
    ...
    case SYMBOL_SWAP_MODE_POINTS:
    {
        ...
        if (swap_long > 0) UnitCost_long = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_PROFIT);
        else UnitCost_long = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_LOSS);
        if (swap_short > 0) UnitCost_short = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_PROFIT);
        else UnitCost_short = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_LOSS);
    ...
    case SYMBOL_SWAP_MODE_CURRENCY_MARGIN:
    {
        ...
        {
            base_or_margin_currency = SymbolInfoString(SymbolForTrading, SYMBOL_CURRENCY_BASE);
            OutputSwapsType = TRANSLATION_LABEL_BASE_CURRENCY + " (" + base_or_margin_currency + ")";
        }
        else
        {
            base_or_margin_currency = SymbolInfoString(SymbolForTrading, SYMBOL_CURRENCY_MARGIN);
        ...
        else
        {
            ...
            {
                double CCC = CalculateAdjustment(Profit, base_or_margin_currency, AccountCurrency, SymbolForTrading);
    ...
    case SYMBOL_SWAP_MODE_INTEREST_CURRENT:
    {
        ...
        if ((CalcMode == SYMBOL_CALC_MODE_CFD) || (CalcMode == SYMBOL_CALC_MODE_CFDINDEX) || (CalcMode == SYMBOL_CALC_MODE_CFDLEVERAGE))
        {
            ...
            else symbol_cost_1_lot_loss = ContractSize * SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
            symbol_cost_1_lot_profit = symbol_cost_1_lot_loss;
            if (ProfitCurrency != AccountCurrency)
            {
                double CCC_loss = CalculateAdjustment(Loss, ProfitCurrency, AccountCurrency, SymbolForTrading);
                double CCC_profit = CalculateAdjustment(Profit, ProfitCurrency, AccountCurrency, SymbolForTrading);
                // Adjust the unit cost.
        ...
        else // Non-CFD.
        {
            ...
            // Calculate it by current price of contract - it is impossible to determine potential future price.
            else symbol_cost_1_lot_loss = UnitCost_loss * SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) / TickSize;
            // For profit (positive swap):
            if ((sets.EntryType == Pending) || (sets.EntryType == StopLimit))
                symbol_cost_1_lot_profit = UnitCost_profit * sets.EntryLevel / TickSize;
            // Calculate it by current price of contract - it is impossible to determine potential future price.
            else symbol_cost_1_lot_profit = UnitCost_profit * SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) / TickSize;
        }
    ...
    {
        OutputSwapsType = TRANSLATION_LABEL_PROFIT_CURRENCY + " (" + ProfitCurrency + ")";
        // Simple case:
        if (AccountCurrency == ProfitCurrency)
        {
            swap_long_1_lot = swap_long;
        ...
        // Need to find the current PRO/ACC or ACC/PRO rate.
        else
        {
            // Simple case - use Symbol rates for conversion:
            ...
            // Go through Market Watch trying to find the currency pair with both ProfitCurrency and AccountCurrency in it.
            else
            {
                double CCC = CalculateAdjustment(Profit, ProfitCurrency, AccountCurrency, SymbolForTrading);
                swap_long_1_lot = swap_long * CCC;
                swap_short_1_lot = swap_short * CCC;
            }
        }
...
void DrawLineLabel(const string label, const string text, const double price, bool above = false, int font_size_modifier = 0)
{
    ...
    if (ExtDialog.DPIScale() > 1) w = (int)MathRound((double)w * 1.1); // Adjustment for DPI-scaled offset from the right-hand scale.
...
void CalculateSymbolLeverage()
{
    ...
    double lotValue = SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK) / TickSize * UnitCost_loss;
...
void CalculateUnitCost(double &UnitCost_loss, double &UnitCost_profit)
{
    ...
    if ((CalcMode != SYMBOL_CALC_MODE_FOREX) && (CalcMode != SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE) && (CalcMode != SYMBOL_CALC_MODE_FUTURES) && (CalcMode != SYMBOL_CALC_MODE_EXCH_FUTURES) && (CalcMode != SYMBOL_CALC_MODE_EXCH_FUTURES_FORTS))
    {
        ...
        if (ProfitCurrency != AccountCurrency)
        {
            double CCC = CalculateAdjustment(Loss, ProfitCurrency, AccountCurrency, SymbolForTrading);
            // Adjust the unit cost.
            UnitCost_loss *= CCC;
            CCC = CalculateAdjustment(Profit, ProfitCurrency, AccountCurrency, SymbolForTrading);
            UnitCost_profit *= CCC;
        }
    }
    // With Forex instruments, tick value already equals 1 unit cost.
    ...
    {
        UnitCost_loss = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_LOSS);
        UnitCost_profit = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_TICK_VALUE_PROFIT);
...
double CalculateCommission()
{
    ...
    if (sets.CommissionType == COMMISSION_PERCENT) // Commission in %. Tricky.
    {
        ...
        double contract_size = SymbolInfoDouble(SymbolForTrading, SYMBOL_TRADE_CONTRACT_SIZE);
        ...
        if ((CalcMode == SYMBOL_CALC_MODE_FOREX) || (CalcMode == SYMBOL_CALC_MODE_FOREX_NO_LEVERAGE)) // Forex.
        {
            ...
            else
            {
                ...
                {
                    ContractValue = contract_size * SymbolInfoDouble(SymbolForTrading, SYMBOL_ASK);
                }
                else // E.g., GBP/JPY.
                {
                    // Conversion needed:
                    double ccc = CalculateAdjustment(Loss, BaseCurrency, AccountCurrency, SymbolForTrading);
...
double AdjustPositionSizeByMinMaxStep(double ps)
{
    if (ps < MinLot) ps = MinLot;
    else if ((ps > MaxLot) && (!SurpassBrokerMaxPositionSize)) ps = MaxLot;
    double steps = 0;
    if (LotStep != 0) steps = ps / LotStep;
    if (MathAbs(MathRound(steps) - steps) < 0.00000001) steps = MathRound(steps);
